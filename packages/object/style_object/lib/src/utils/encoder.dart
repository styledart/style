...
class StyleObjectEncoder extends Converter<StyleObject, ByteData> {
  ...
  ByteData convert(StyleObject input) {
    var bytes =
        ByteDataWriter(ByteData(input.getLength(rootKey) + k16BitLength));
    input.write(bytes, rootKey, true);
    return bytes.byteData;
  }
}

class StyleObjectDecoder extends Converter<ByteData, Map<int, dynamic>> {
  ...
  @override
  Map<int, dynamic> convert(ByteData input) {
    var reader = ByteDataReader(input);
    var read = rootKey.read(reader, false);
